...
namespace uuids {
    ...
    namespace detail {
        ...
        [[nodiscard]] constexpr inline unsigned char hex2char(
            TChar const ch) noexcept {
            if (ch >= static_cast<TChar>('0') &&
                ch <= static_cast<TChar>('9')) {
                return static_cast<unsigned char>(ch - static_cast<TChar>('0'));
            }
            if (ch >= static_cast<TChar>('a') &&
                ch <= static_cast<TChar>('f')) {
                return static_cast<unsigned char>(10 + ch -
                                                  static_cast<TChar>('a'));
            }
            if (ch >= static_cast<TChar>('A') &&
                ch <= static_cast<TChar>('F')) {
        ...
        [[nodiscard]]
        constexpr std::basic_string_view<typename StringType::value_type,
        ...
        class sha1 {
            public:
                ...
                void process_block(void const* const start,
        ...
        template<>
        inline constexpr wchar_t guid_encoder<wchar_t>[17] =
            L"0123456789abcdef";
    }  // namespace detail
    ...
    // of the timestamp. time_mid	                  unsigned short
    // 4 - 5	   The middle field of the timestamp. time_hi_and_version
    // unsigned short	   6 - 7	   The high field of the timestamp
    // multiplexed with the version number. clock_seq_hi_and_reserved	unsigned
    // small	   8	      The high field of the clock sequence multiplexed
    // with the variant. clock_seq_low	            unsigned small	   9
    // The low field of the clock sequence. node character	      10 - 15
    ...
    enum class uuid_variant {
        ...
        // Computing System 1.5 UUID format) N bit pattern: 0xxx > the first 6
        // octets of the UUID are a 48-bit timestamp (the number of 4
        // microsecond units of time since 1 Jan 1980 UTC); > the next 2 octets
        // are reserved; > the next octet is the "address family"; > the final 7
        // octets are a 56-bit host ID in the form specified by the address
        // family
        ncs,
    ...
    enum class uuid_version {
        ...
        random_number_based = 4,  // The randomly or pseudo-randomly generated
                                  // version specified in RFS 4122
        name_based_sha1 =
            5  // The name-based version specified in RFS 4122 with SHA1 hashing
    };

    // Forward declare uuid & to_string so that we can declare to_string as a
    // friend later.
    class uuid;
    ...
    class uuid {
        public:
            ...
            explicit uuid(span<value_type, 16> bytes) {
                std::copy(std::cbegin(bytes), std::cend(bytes),
                          std::begin(data));
            }
            ...
            [[nodiscard]] constexpr static std::optional<uuid> from_string(
                ...
                for (size_t i = hasBraces; i < str.size() - hasBraces; ++i) {
                    ...
                    if (firstDigit) {
                        data[index] =
                            static_cast<uint8_t>(detail::hex2char(str[i]) << 4);
                        firstDigit = false;
                    } else {
        ...
        private:
            ...
            friend std::basic_string<CharT, Traits, Allocator> to_string(
                uuid const& id);
    ...
    [[nodiscard]] inline std::basic_string<CharT, Traits, Allocator> to_string(
        uuid const& id) {
        std::basic_string<CharT, Traits, Allocator> uustr{
            detail::empty_guid<CharT>};
    ...
    static uuid uuid_namespace_dns{{0x6b, 0xa7, 0xb8, 0x10, 0x9d, 0xad, 0x11,
                                    0xd1, 0x80, 0xb4, 0x00, 0xc0, 0x4f, 0xd4,
                                    0x30, 0xc8}};

    // Name string is a URL
    static uuid uuid_namespace_url{{0x6b, 0xa7, 0xb8, 0x11, 0x9d, 0xad, 0x11,
                                    0xd1, 0x80, 0xb4, 0x00, 0xc0, 0x4f, 0xd4,
                                    0x30, 0xc8}};

    // Name string is an ISO OID (See https://oidref.com/,
    // https://en.wikipedia.org/wiki/Object_identifier)
    static uuid uuid_namespace_oid{{0x6b, 0xa7, 0xb8, 0x12, 0x9d, 0xad, 0x11,
                                    0xd1, 0x80, 0xb4, 0x00, 0xc0, 0x4f, 0xd4,
                                    0x30, 0xc8}};
    ...
    static uuid uuid_namespace_x500{{0x6b, 0xa7, 0xb8, 0x14, 0x9d, 0xad, 0x11,
                                     0xd1, 0x80, 0xb4, 0x00, 0xc0, 0x4f, 0xd4,
                                     0x30, 0xc8}};
    ...
    class uuid_name_generator {
        ...
        private:
            ...
            void process_characters(
                std::basic_string_view<CharT, Traits> const str) {
                for (uint32_t c : str) {
...
namespace std {
    ...
    struct hash<uuids::uuid> {
            ...
            [[nodiscard]] result_type operator()(
                argument_type const& uuid) const {
                ...
                if constexpr (sizeof(result_type) > 4) {
                    ...
                    uint64_t hash64 = l ^ h;
                    return result_type(uint32_t(hash64 >> 32) ^
                                       uint32_t(hash64));
                }
#endif
            }
